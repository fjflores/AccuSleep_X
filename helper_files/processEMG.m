...
if samplesPerEpoch == floor( samplesPerEpoch )
    processedEMG = log( rms( reshape(...
        EMG, samplesPerEpoch, length( EMG ) / samplesPerEpoch ) ) );

else
    processedEMG = zeros(1,floor(length(EMG)/samplesPerEpoch));

    for i = 1:length( processedEMG )
        processedEMG( i ) = log( rms(...
            EMG( floor( ( i - 1 ) * samplesPerEpoch + 1 )...
            : floor( i * samplesPerEpoch ) ) ) );
